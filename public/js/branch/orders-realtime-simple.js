...
function updateOrderCardStatus(orderCard, newStatus, message) {
    ...
    if (statusBadge) {
        ...
        statusBadge.className = `px-2 py-1 text-xs font-medium rounded-full status-badge ${getStatusColor(newStatus)}`;
    }
    ...
    if (newStatus === 'driver_found' && message && message.driver) {
        addDriverInfo(orderCard, message.driver);
    }
}
...
function addDriverInfo(orderCard, driver) {
    ...
    const buttonsContainer = orderCard.querySelector('.flex.gap-2');
    if (buttonsContainer) {
        orderCard.insertBefore(driverInfo, buttonsContainer);
    }
}
