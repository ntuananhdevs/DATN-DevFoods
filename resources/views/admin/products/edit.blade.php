                ...
                <div class="space-y-5 md:col-span-2">
                    ...
                    <div>
                        ...
                        <textarea id="ingredients" name="ingredients" rows="3"
                            ...
                            class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm resize-none @error('ingredients') border-red-500 @enderror">{{ old('ingredients', $product->ingredients) }}</textarea>
                        @error('ingredients')
                            <div class="text-red-500 text-xs mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                                    ...
                                    <div id="image-placeholder"
                                        ...
                                        @if($product->primary_image)
                                            <div id="main-image-preview" class="absolute inset-0 w-full h-full">
                                                <img src="{{ str_starts_with($product->primary_image, 'http') ? $product->primary_image : asset('storage/' . $product->primary_image) }}" alt="Current main image"
                                    ...
                                    @if($product->images && $product->images->count() > 0)
                                        @foreach($product->images as $image)
                                            <div class="relative group">
                                                <img src="{{ str_starts_with($image->image_path, 'http') ? $image->image_path : asset('storage/' . $image->image_path) }}" alt="Product image"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" id="attribute_name_0" name="attributes[0][name]" placeholder="VD: Kích thước"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" value="{{ old('attributes.0.name', $product->attributes->first()->name ?? '') }}">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3 p-3 border border-dashed border-gray-300 rounded-md">
                                        <div class="md:col-span-2">
                                            ...
                                            <input type="text" id="attribute_value_0_0" name="attributes[0][values][0][value]" placeholder="VD: Nhỏ"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-xs" value="{{ old('attributes.0.values.0.value', $product->attributes->first()?->values->first()?->value ?? '') }}">
                                        ...
                                        <div>
                                            ...
                                            <input type="number" id="attribute_price_0_0" name="attributes[0][values][0][price]" placeholder="0" step="any"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-xs" value="{{ old('attributes.0.values.0.price', $product->attributes->first()?->values->first()?->price ?? '') }}">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Restore topping data (prioritize old data if validation fails, otherwise use product data)
        @if (old('toppings'))
        ...
        @elseif ($product->toppings && $product->toppings->count() > 0)
            const productToppings = @json($product->toppings);
            if (Array.isArray(productToppings)) {
                productToppings.forEach((topping, index) => {
                    const toppingGroup = createToppingGroup(index);
                    toppingsContainer.appendChild(toppingGroup);
                    const nameInput = toppingGroup.querySelector(
                        `input[name="toppings[${index}][name]"]`);
                    ...
                    if (nameInput) nameInput.value = topping.name || '';
                    if (priceInput) priceInput.value = topping.price || '';
                    if (availableInput) availableInput.checked = topping.available === 1;
                });
                toppingCount = productToppings.length;
            }
        @endif

        // Restore attribute data (prioritize old data if validation fails, otherwise use product data)
        @if (old('attributes'))
        ...
        @elseif ($product->attributes && $product->attributes->count() > 0)
            const productAttributes = @json($product->attributes->load('values'));
            if (Array.isArray(productAttributes)) {
                ...
                attributeCount = Math.max(productAttributes.length, 1); // Ensure at least 1
            }
        @endif
        ...
        console.log('Product attributes:', @json($product->attributes));
        console.log('Product toppings:', @json($product->toppings));
    });
</script>

<!-- Debug: Display product data -->
@if(config('app.debug'))
<div class="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-4">
    <strong>Debug Info:</strong>
    <p>Product ID: {{ $product->id }}</p>
    <p>Attributes count: {{ $product->attributes ? $product->attributes->count() : 'null' }}</p>
    <p>Toppings count: {{ $product->toppings ? $product->toppings->count() : 'null' }}</p>
    @if($product->attributes && $product->attributes->count() > 0)
        <p>First attribute: {{ $product->attributes->first()->name ?? 'No name' }}</p>
        <p>First attribute values count: {{ $product->attributes->first()->values ? $product->attributes->first()->values->count() : 'null' }}</p>
    @endif
    @if($product->toppings && $product->toppings->count() > 0)
        <p>First topping: {{ $product->toppings->first()->name ?? 'No name' }}</p>
    @endif
</div>
@endif
