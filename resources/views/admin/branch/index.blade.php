...
@section('scripts')
    <script>
        ...
        async function loadBranches(page = 1, search = currentSearch, append = false) {
            ...
            try {
                const response = await fetch(
                    `{{ route('admin.branches.index') }}?page=${page}&search=${encodeURIComponent(search)}`, {
                        headers: {
            ...
            dtmodalCreateModal({
                ...
                onConfirm: async () => {
                    try {
                        ...
                        if (!response.ok) {
                            ...
                            if (response.status === 404) {
                                ...
                                if (errorData.message.includes('quản lý HOẠT ĐỘNG')) {
                                    errorMessage =
                                        'Không thể thay đổi trạng thái chi nhánh vì chi nhánh này có quản lý đang hoạt động';
                                }
                            }
                        ...
                        if (data.success) {
                            button.classList.remove('bg-green-100', 'text-green-700', 'bg-red-100',
                                'text-red-700');
                            button.classList.add(data.data.active ? 'bg-green-100' : 'bg-red-100', data
                                .data.active ? 'text-green-700' : 'text-red-700');
                            button.innerHTML =
                                `<i class="fas ${data.data.active ? 'fa-check' : 'fa-times'} mr-1"></i> ${data.data.status_text}`;
                ...
                onConfirm: async () => {
                    try {
                        ...
                        if (data.success) {
                            ...
                            updateBulkActionsVisibility();
                            document.getElementById('selectAllButton').querySelector('span').textContent =
                                'Chọn tất cả';
                        } else {
                            throw new Error(data.message || messages.errorMessage);
                        }
        ...
        function showToast(type, message) {
            ...
            toast.className =
                `toast toast-${type} fixed bottom-4 right-4 p-4 rounded-md shadow-md bg-${type === 'success' ? 'green-500' : 'red-500'} text-white`;
