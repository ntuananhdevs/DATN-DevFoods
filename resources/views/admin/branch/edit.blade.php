        ...
        <div class="header-content">
            <div class="header-left">
                ...
                <div class="header-text">
                    ...
                    <p>Cập nhật thông tin chi nhánh</p>
                </div>
            </div>
            <div class="header-actions">
                ...
                <button type="submit" form="branchForm" class="btn btn-primary">
            ...
            <div class="space-y-6">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div id="existingImages" class="image-preview-container {{ $branch->images->isEmpty() ? 'hidden' : '' }}">
                            <h4>Hình ảnh hiện tại:</h4>
                            <div id="existingImagesContainer" class="image-preview-list">
                                @foreach($branch->images as $index => $image)
                                    <div class="image-preview-item" data-image-id="{{ $image->id }}">
                                        <img src="{{ Storage::url($image->image_path) }}" class="image-preview-img" alt="Hình ảnh chi nhánh">
                                        <div class="image-preview-overlay">
                                            <div class="image-preview-actions">
                                                <button type="button" class="image-preview-btn remove-btn" data-image-id="{{ $image->id }}" aria-label="Xóa ảnh">
                                                    <i class="fas fa-trash-alt"></i>
                                                </button>
                                            </div>
                                        </div>
                                        <input type="hidden" name="delete_images[]" class="delete-image-input hidden" value="{{ $image->id }}">
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div id="imagePreview" class="image-preview-container hidden">
                            ...
                            <div id="previewContainer" class="image-preview-list"></div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="primary_image" name="primary_image" aria-label="Chọn ảnh chính">
                                @if($branch->images->count() > 0)
                                    @foreach($branch->images as $index => $image)
                                        <option value="{{ $image->id }}" {{ $image->is_primary ? 'selected' : '' }}>
                        ...
                        <div id="existingCaptionsContainer" class="{{ $branch->images->isEmpty() ? 'hidden' : '' }}">
                            <h4 class="mb-3">Mô tả cho ảnh hiện tại:</h4>
                            <div id="existingCaptionInputs">
                                @foreach($branch->images as $index => $image)
                                    <div class="form-group" data-image-id="{{ $image->id }}">
                                        <label class="form-label">Mô tả ảnh hiện tại {{ $index + 1 }}:</label>
                                        <input type="text" 
                                               class="form-control" 
                    ...
                    <div class="card-body">
                        <div class="preview-card">
                            <div class="preview-header">
                                <i class="fas fa-building text-primary"></i>
                                <h4 class="preview-title" id="previewName">{{ old('name', $branch->name) }}</h4>
                            </div>
                            ...
                            <div class="preview-item">
                                <i class="fas fa-phone text-primary"></i>
                                <span id="previewPhone">{{ old('phone', $branch->phone) }}</span>
                            </div>
                            <div class="preview-item {{ $branch->email ? '' : 'hidden' }}" id="previewEmailContainer">
                                <i class="fas fa-envelope text-primary"></i>
                                <span id="previewEmail">{{ old('email', $branch->email) ?: 'Email' }}</span>
                            </div>
                            <div class="preview-item" id="previewManagerContainer">
                                <i class="fas fa-user-tie text-primary"></i>
                                <span id="previewManager">
                                    {{ old('manager_user_id', $branch->manager_user_id) ? $availableManagers->firstWhere('id', old('manager_user_id', $branch->manager_user_id))?->full_name : 'Chưa chọn quản lý' }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="preview-hours">
                            <div class="preview-hour opening">
                                ...
                                <div class="preview-hour-value opening" id="previewOpeningHour">{{ old('opening_hour', $branch->opening_hour) }}</div>
                            </div>
                            <div class="preview-hour closing">
                                <i class="fas fa-moon text-danger"></i>
                                <div class="preview-hour-label">Đóng cửa</div>
                                <div class="preview-hour-value closing" id="previewClosingHour">{{ old('closing_hour', $branch->closing_hour) }}</div>
                            </div>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const existingImagesSection = document.getElementById('existingImages');
        ...
        let map;
        let marker;
        let existingImages = {!! json_encode($branch->images->map(function($image) {
            return [
                'id' => $image->id,
                'image_path' => asset($image->image_path), // Use asset() helper to get full URL
                'caption' => $image->caption,
                'is_primary' => (bool)$image->is_primary // Cast to boolean
            ];
        })->values()) !!}; // Use values() to reindex array
        ...
        function initForm() {
            ...
            form.addEventListener('submit', function(e) {
                ...
                // Xử lý giá trị primary image
                const primaryImageSelect = document.getElementById('primary_image');
                let primaryValue = primaryImageSelect.value;
                
                // Chuyển đổi giá trị thành integer
                if (primaryValue.startsWith('new_')) {
                    // Nếu là ảnh mới, lấy index
                    primaryValue = parseInt(primaryValue.replace('new_', ''));
                } else {
                    // Nếu là ảnh hiện có hoặc '0', chuyển thành integer
                    primaryValue = parseInt(primaryValue) || 0;
                }
                
                // Cập nhật giá trị trong select
                primaryImageSelect.value = primaryValue;
            });
            ...
            document.querySelectorAll('.remove-btn').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    const deleteInput = previewItem.querySelector('.delete-image-input');
                    deleteInput.classList.remove('hidden');
                    previewItem.classList.add('hidden');
                    
                    // Remove the corresponding caption input
                    const captionGroup = existingCaptionInputs.querySelector(`.form-group[data-image-id="${imageId}"]`);
                    if (captionGroup) {
                        captionGroup.remove();
                    }
                    ...
                    // Hide sections if no images remain
        ...
        function handleImageUpload(event) {
            ...
            if (totalImages > maxImages) {
                alert(`Tổng số hình ảnh (hiện tại + mới) không được vượt quá ${maxImages}.`);
                imagesInput.value = '';
                return;
            }
        ...
        function displayImagePreviews(files) {
            ...
            files.forEach((file, index) => {
                ...
                reader.onload = function(e) {
                    ...
                    captionInput.name = `captions[new_${index}]`;
        ...
        function updatePrimaryImage() {
            const selectedId = primaryImageSelect.value;
            ...
            const selectedPreview = document.querySelector(`.image-preview-item[data-image-id="${selectedId}"]`) || 
                                   document.querySelector(`.image-preview-item[data-index="${selectedId.replace('new_', '')}"]`);
            if (selectedPreview) {
        ...
        function updatePrimaryImageSelect() {
            const currentPrimary = primaryImageSelect.value;
            ...
            existingImages.forEach((image, index) => {
                ...
                option.textContent = `Ảnh hiện tại ${index + 1}`;
                if (image.is_primary) {
            ...
            if (!primaryImageSelect.options.length) {
                const option = document.createElement('option');
                option.value = '0';
            ...
            // Set default selection if no current primary
            if (!currentPrimary || !primaryImageSelect.querySelector(`option[value="${currentPrimary}"]`)) {
                if (existingImages.length > 0) {
                    const primaryImage = existingImages.find(img => img.is_primary);
                    primaryImageSelect.value = primaryImage ? primaryImage.id : existingImages[0].id;
                } else if (uploadedImages.length > 0) {
                    primaryImageSelect.value = 'new_0';
                } else {
