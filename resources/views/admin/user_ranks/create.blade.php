...
@section('description', 'Tạo hạng thành viên mới với các quyền lợi và điều kiện')
...
<div class="fade-in flex flex-col gap-4 pb-4">
    ...
    <form action="{{ route('admin.user_ranks.store') }}" method="POST" id="tierForm">
        @csrf
                ...
                <div class="form-card">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="form-group">
                            ...
                            <input type="text" id="name" name="name" class="form-input @error('name') error @enderror"
                                value="{{ old('name') }}" placeholder="Ví dụ: Vàng, Bạc, Kim cương..." required>
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="slug" name="slug" class="form-input @error('slug') error @enderror"
                                value="{{ old('slug') }}" placeholder="vi-du: vang, bac, kim-cuong..." required>
                            @error('slug')
                            <div class="error-message">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="color-picker-container">
                                ...
                                <input type="text" id="colorText" class="form-input"
                                    value="{{ old('color', '#CD7F32') }}" placeholder="#CD7F32">
                            </div>
                            @error('color')
                            <div class="error-message">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="form-group">
                        ...
                        <div class="icon-grid" id="iconGrid">
                            ...
                            <div class="icon-option {{ old('icon', '🥉') === $iconOption ? 'selected' : '' }}" data-icon="{{ $iconOption }}">
                                {{ $iconOption }}
                            </div>
                            @endforeach
                        </div>
                        ...
                        <div class="range-group">
                            <div>
                                <input type="number" id="min_spending" name="min_spending" class="form-input @error('min_spending') error @enderror"
                                    value="{{ old('min_spending', 0) }}" min="0" step="0.01" placeholder="Tối thiểu" required>
                                @error('min_spending')
                                <div class="error-message">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="range-group">
                            <div>
                                <input type="number" id="min_orders" name="min_orders" class="form-input @error('min_orders') error @enderror"
                                    value="{{ old('min_orders', 0) }}" min="0" placeholder="Tối thiểu" required>
                                @error('min_orders')
                                <div class="error-message">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="form-group">
                            ...
                            <input type="number" id="discount_percentage" name="discount_percentage" class="form-input @error('discount_percentage') error @enderror"
                                value="{{ old('discount_percentage', 0) }}" min="0" max="100" step="0.01" placeholder="0">
            ...
            <div class="lg:col-span-1">
                <div class="tier-preview">
                    ...
                    <div class="tier-preview-description" id="previewSlug">
                        Slug: {{ old('slug', 'ten-hang') }}
                    </div>

                    <div class="tier-preview-stats">
                        <div class="tier-preview-stat">
                            <div class="tier-preview-stat-value" id="previewDiscount">{{ old('discount_percentage', 0) }}%</div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="text-sm text-gray-600">
                            <div id="previewSpentRange">Chi tiêu: {{ number_format(old('min_spending', 0)) }} đ</div>
                            <div id="previewOrderRange">Đơn hàng: {{ old('min_orders', 0) }}</div>
                        </div>
                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateSpentRange() {
            const minSpent = document.getElementById('min_spending').value || 0;
            document.getElementById('previewSpentRange').textContent = 'Chi tiêu: ' + new Intl.NumberFormat('vi-VN').format(minSpent) + ' đ';
        }
        ...
        function updateBenefitsPreview() {
            ...
            if (previewBenefits.children.length === 0) {
                const div = document.createElement('div');
                div.textContent = '• Chưa có quyền lợi nào';
