...
<style>
  ...
  .status-active .status-label:hover {
  ...
  .status-inactive .status-label:hover {
...
<div class="data-table-wrapper">
  ...
  <div class="data-table-main-header">
    <div class="data-table-brand">
      ...
      <h1 class="data-table-title">Chỉnh sửa người dùng</h1>
    </div>
  </div>
  <div class="container">
      ...
      <form action="{{ route('admin.users.update', $user->id) }}" method="POST" enctype="multipart/form-data" id="userForm">
        ...
        <div class="data-table-header" style="border-bottom: none; margin-left: -1.5rem; margin-right: -1.5rem;">
          <div style="display: flex; align-items: center; gap: 0.5rem;">
            ...
            <span class="badge bg-primary" style="font-size: 0.75rem; padding: 0.25rem 0.5rem; border-radius: 9999px;">
              Đang chỉnh sửa
            </span>
          </div>
          <div style="display: flex; align-items: center; gap: 0.5rem; margin-top: 0.5rem; color: #64748b; font-size: 0.875rem;">
            <i class="fas fa-info-circle"></i>
            <span>Cập nhật thông tin chi tiết cho người dùng <strong>{{ $user->full_name }}</strong></span>
          </div>
        </div>
        <div class="form-group">
          <label for="user_name">Tên đăng nhập</label>
          <input type="text" class="@error('user_name') is-invalid @enderror" id="user_name" name="user_name"
            value="{{ old('user_name', $user->user_name) }}" placeholder="Tên đăng nhập">
          @error('user_name')
        ...
        <div class="form-group">
          ...
          <input type="text" class="@error('full_name') is-invalid @enderror" id="full_name" name="full_name"
            value="{{ old('full_name', $user->full_name) }}" placeholder="Họ và tên">
          @error('full_name')
          <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
        ...
        <div class="form-group">
          <label for="active" class="form-label">Trạng thái tài khoản</label>
          <div class="status-group">
            <label class="status-option {{ old('active', $user->active) == '1' ? 'status-active' : '' }}" id="status-active-label">
              <input
                type="radio"
                class="status-radio"
                id="active-on"
            ...
            <label class="status-option {{ old('active', $user->active) == '0' ? 'status-inactive' : '' }}" id="status-inactive-label">
              <input
                type="radio"
                class="status-radio"
                id="active-off"
    ...
    <div class="column column-right">
      ...
      @if($user->avatar)
      <div class="current-avatar-container" style="margin-top: 2rem; padding: 1rem; border: 1px solid var(--border-color); border-radius: var(--radius-md);">
        <div class="avatar-title" style="font-size: 0.875rem; font-weight: 500; color: var(--text-primary); margin-bottom: 1rem;">
          Ảnh hiện tại
        </div>
        <img src="{{ asset('storage/'.$user->avatar) }}" alt="Avatar hiện tại" class="current-avatar"
          style="width: 100%; max-height: 300px; object-fit: contain; border-radius: var(--radius-md);">
      </div>
      @endif
      @error('avatar')
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      const statusActiveLabel = document.getElementById('status-active-label');
      const statusInactiveLabel = document.getElementById('status-inactive-label');

      // Add event listeners to radio buttons
      activeRadios.forEach(radio => {
        radio.addEventListener('change', function() {
          // Reset both labels
          statusActiveLabel.classList.remove('status-active');
          statusInactiveLabel.classList.remove('status-inactive');

          // Apply correct styling based on selected value
          if (this.value === '1') {
            statusActiveLabel.classList.add('status-active');
          } else {
            statusInactiveLabel.classList.add('status-inactive');
          }
        });
      });
    });
