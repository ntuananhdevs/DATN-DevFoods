...
<main class="container">
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-extrabold mb-1">Thêm Topping Mới</h1>
            <p class="text-gray-500">Nhập thông tin chi tiết để tạo topping mới</p>
        </div>
    ...
    <form id="add-topping-form" class="space-y-8" action="{{ route('admin.toppings.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <section class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
            <header class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-500 text-sm mt-1">Nhập thông tin cơ bản của topping</p>
                </div>
            </header>
                ...
                <div class="space-y-5 md:col-span-2">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                            ...
                            class="mt-1 block w-full px-4 py-3 rounded-md border border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm resize-none @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="text-red-500 text-xs mt-1">{{ $message }}</div>
                        @enderror
                    </div>
        ...
        <div class="flex items-center justify-end space-x-4">
            ...
            <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors">
                <i class="fas fa-save mr-2"></i>Lưu Topping
            </button>
        </div>
    </form>
</main>
