...
@section('content')
                    ...
                    <div class="chat-item conversation-item {{ $loop->first ? 'active' : '' }}  relative"
                        ...
                        <div class="flex items-center gap-3 w-full min-w-0">
                            ...
                            @if ($conv->unread_count ?? 0)
                                <span class="unread-badge ml-2 absolute right-2 bottom-2">{{ $conv->unread_count }}</span>
                            @endif
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const userId = {{ auth()->guard('manager')->user()->id }};
            ...
            if (selectedConversationId && selectedConversationId !== 'null') {
                ...
                window.chatInstance = new BranchChat({
            ...
            document.querySelectorAll('.conversation-item').forEach(item => {
                item.addEventListener('click', function() {
                    ...
                    // Cập nhật lại header, info panel
                    const customerName = this.getAttribute('data-customer-name');
                    const customerEmail = this.getAttribute('data-customer-email');
                    const branchName = this.getAttribute('data-branch-name');
                    document.getElementById('chat-header-name').textContent = customerName;
                    document.getElementById('chat-header-email').textContent = customerEmail;
                    document.getElementById('chat-header-avatar').textContent = customerName.charAt(
                        0).toUpperCase();
                    document.getElementById('chat-info-name').textContent = customerName;
                    document.getElementById('chat-info-email').textContent = customerEmail;
                    document.getElementById('chat-info-avatar').textContent = customerName.charAt(0)
                        .toUpperCase();
                    document.getElementById('chat-info-branch').textContent =
                        `Chi nhánh: ${branchName}`;
                    // Khởi tạo lại BranchChat với conversation mới
                    window.chatInstance = new BranchChat({
                        conversationId: conversationId,
                        userId: userId,
                        api: {
                            getMessages: getApiGetMessagesUrl(conversationId),
                            send: '/branch/chat/send-message',
                            updateStatus: '/branch/chat/api/update-status',
                        },
                        messageInputSelector: '#chat-input-message',
