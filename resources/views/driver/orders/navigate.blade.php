...
<body>
    ...
    <script>
        ...
        async function calculateRoute() {
            ...
            if (!orderData.guest_longitude || !orderData.guest_latitude) {
                dtmodalShowToast("error", {
                    title: "Lỗi tọa độ",
                    message: "Không có tọa độ điểm đến hợp lệ."
                });
                return;
            }
            ...
            try {
                // Use Mapbox Directions API with proper parameters
                ...
                dtmodalShowToast("error", {
                    ...
                    message: error.message || "Không thể tính toán đường đi. Vui lòng thử lại."
                });
                
                // Fallback: enable navigation anyway with basic distance calculation
                document.getElementById('start-navigation-btn').disabled = false;
                document.getElementById('status-text').textContent = "Sẵn sàng điều hướng (không có đường đi chi tiết)";
                ...
                document.getElementById('total-distance').textContent = formatDistance(distance * 1000);
                document.getElementById('total-time').textContent = formatTime(time * 60);
            ...
            if ("geolocation" in navigator) {
                watchId = navigator.geolocation.watchPosition(
                    function(position) {
                        ...
                        // Update remaining distance and instructions
                    ...
                    function(error) {
                        ...
                        dtmodalShowToast("warning", {
                            title: "Cảnh báo",
                            message: "Không thể cập nhật vị trí. Kiểm tra GPS."
                        });
                    },
                    {
                        enableHighAccuracy: true,
        ...
        function updateCurrentInstruction() {
            ...
            if (route && route.legs && route.legs[0] && route.legs[0].steps) {
                ...
                let currentStep = steps[0]; // Default to first step
                
                // Simple logic to find current step (can be improved)
                ...
                if (currentStep.maneuver && currentStep.maneuver.instruction) {
                    instructionElement.textContent = currentStep.maneuver.instruction;
                } else {
                    instructionElement.textContent = "Tiếp tục theo đường đi";
                }
            } else {
                ...
                if (distance < 0.1) { // Less than 100m
                    instructionElement.textContent = "Bạn đã đến gần điểm đến";
                } else {
                    instructionElement.textContent = "Tiếp tục di chuyển đến điểm đến";
                }
            }
        }
        ...
        function updateNavigationInfo() {
            ...
            if (distance < 0.05 && !arrivedNotified) {
                arrivedNotified = true;
                dtmodalShowToast("success", {
                    title: "Đã đến nơi",
                    message: "Bạn đã đến gần điểm đến. Hãy liên hệ khách hàng."
                });
                
                // Optionally stop navigation tracking
                if (watchId) {
        ...
        function callCustomer() {
            if (orderData && orderData.customer_phone) {
                dtmodalCreateModal({
                    ...
                    message: `Bạn có muốn gọi cho ${orderData.customer_name}?<br><strong>${orderData.customer_phone}</strong>`,
