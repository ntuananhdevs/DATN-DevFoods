    ...
    <div class="flex items-center gap-4">
        ...
        <div class="relative" x-data="{ open: false }">
            ...
            <div x-show="open" @click.away="open = false"
                class="absolute right-0 mt-2 w-96 rounded-md border bg-popover text-popover-foreground shadow-md overflow-hidden z-50"
                style="display: none;">
                <div class="p-2 max-h-[calc(100vh-100px)] overflow-y-auto custom-scrollbar flex flex-col"
                    ...
                    <div class="space-y-1 flex-1 overflow-y-auto" id="branch-notification-list">
                        @include('partials.branch._notification_items', [
            ...
            <button @click="open = !open"
                ...
                <div class="relative h-8 w-8 rounded-full bg-muted">
                    @if (Auth::guard('manager')->user()->avatar)
                        <img src="{{ Storage::url(Auth::guard('manager')->user()->avatar) }}"
                            alt="{{ Auth::guard('manager')->user()->full_name }}"
                    ...
                    <div class="px-2 py-1.5">
                        <div class="flex flex-col space-y-1">
                            <p class="text-sm font-medium leading-none">
                                {{ Auth::guard('manager')->user()->full_name }}</p>
                            <p class="text-xs leading-none text-muted-foreground">
                                {{ Auth::guard('manager')->user()->email }}</p>
                            @if (Auth::guard('manager')->user()->branch)
                                <p class="text-xs leading-none text-primary font-medium">
                                    {{ Auth::guard('manager')->user()->branch->name }}</p>
                            @endif
                        </div>
                    </div>
                    <div class="h-px my-1 bg-muted"></div>
...
<script>
    ...
    window.branchId = "{{ auth('manager')->user()->branch->id ?? '' }}";
    window.currentUserId = "{{ auth('manager')->id() }}";
    window.Echo.channel('branch.' + window.branchId + '.conversations')
