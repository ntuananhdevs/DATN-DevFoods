@foreach($branchNotifications ?? [] as $notification)
    @php
        ...
        $redirectUrl = $orderId ? route('branch.orders.index', ['order' => $orderId]) : '';
    @endphp
    <div class="flex items-start gap-3 px-2 py-2 hover:bg-accent rounded-md {{ $notification->read_at ? '' : 'bg-primary/10 text-primary font-semibold' }}"
        ...
        <div class="flex-1">
            ...
            <p class="text-xs text-muted-foreground">{{ \Carbon\Carbon::parse($notification->created_at)->diffForHumans() }}</p>
        </div>
