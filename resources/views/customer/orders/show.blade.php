...
@section('content')
    ...
    <div class="bg-gradient-to-r from-orange-500 to-red-500 py-8">
        <div class="container mx-auto px-4">
            <div class="flex items-center">
                ...
                <h1 class="text-2xl md:text-3xl font-bold text-white">Chi tiết đơn hàng #{{ $order->order_code ?? $order->id }}</h1>
            </div>
        </div>
    </div>
    <div class="container mx-auto px-4 py-8">
        <div class="flex flex-col gap-8">
            <section class="mb-10">
                <div id="order-details-card" class="bg-white rounded-xl shadow-sm overflow-hidden" data-order-id="{{ $order->id }}">
                    <div class="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between sm:items-start">
                        <div>
                            <h2 class="text-xl font-bold text-gray-900">Đơn hàng #{{ $order->order_code ?? $order->id }}</h2>
                            <div class="flex items-center mt-2 space-x-4 text-gray-500 text-sm">
                                <div class="flex items-center"><i class="far fa-calendar-alt mr-1.5"></i><span>Đặt lúc: {{ $order->order_date->format('H:i - d/m/Y') }}</span></div>
                                @if (in_array($order->status, ['delivered', 'item_received']))
                                    <div id="completed-time" class="flex items-center text-green-600"><i class="fas fa-check-circle mr-1.5"></i><span>Hoàn thành: {{ optional($order->actual_delivery_time)->format('H:i - d/m/Y') }}</span></div>
                                @elseif($order->status != 'cancelled')
                                    <div id="estimated-time" class="flex items-center"><i class="far fa-clock mr-1.5"></i><span>Dự kiến: {{ $order->estimated_delivery_time ? date('H:i', strtotime($order->estimated_delivery_time)) : 'N/A' }}</span></div>
                                @endif
                            </div>
                        </div>
                        ...
                        @if ($currentStepIndex > -1)
                            <div id="progress-tracker">
                                <div class="relative">
                                    ...
                                    <div id="progress-bar" class="absolute top-4 left-0 h-0.5 bg-orange-500 transition-all duration-500" style="width: {{ ($currentStepIndex / (count($progressSteps) - 1)) * 100 }}%;"></div>
                                    <div class="relative flex justify-between">
                                        @foreach ($progressSteps as $key => $stepInfo)
                                            ...
                                            @endphp
                                            <div class="flex flex-col items-center w-1/4 z-10" data-step-key="{{ $key }}">
                                                <div class="progress-icon w-8 h-8 rounded-full flex items-center justify-center text-white text-base {{ $isCompleted ? 'bg-orange-500' : 'bg-gray-300' }}">
                    ...
                    <div class="p-6 border-t bg-gray-50/50 rounded-b-xl">
                        <div id="action-buttons" class="flex flex-wrap gap-3 justify-end">
                            ...
                            @elseif($order->status == 'delivered')
                                ...
                                <form class="receive-order-form" action="{{ route('customer.orders.updateStatus', $order) }}" method="POST">
                                    @csrf
                            ...
                            @elseif($order->status == 'item_received')
                                <a href="#" class="inline-flex items-center justify-center rounded-md text-sm font-medium text-white h-10 px-4 py-2 bg-yellow-500 hover:bg-yellow-600"><i class="fas fa-star mr-2"></i>Đánh giá</a>
                            @endif
                            <button class="inline-flex items-center justify-center rounded-md text-sm font-medium h-10 px-4 py-2 bg-gray-100 text-gray-800 hover:bg-gray-200">Đặt lại đơn hàng</button>
                        </div>
                    </div>
                </div>
            </section>
    ...
    <div id="product-details-modal" class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 transition-opacity duration-300 opacity-0 pointer-events-none">
        <div id="modal-content" class="bg-white rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto transform scale-95 transition-transform duration-300"></div>
    </div>
        ...
        <div class="relative mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3 text-center">
                ...
                <div class="mt-2 px-7 py-3">
                    <p id="action-modal-message" class="text-sm text-gray-500">Bạn có chắc chắn thực hiện thao tác này không?</p>
                </div>
                <div class="items-center px-4 py-3 flex gap-3">
                    <button id="action-abort-btn" class="w-full px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Không</button>
                    <button id="action-confirm-btn" class="w-full px-4 py-2 bg-orange-600 text-white rounded-md hover:bg-orange-700">Đồng ý</button>
                </div>
            </div>
        </div>
    </div>
    <div id="toast-message" class="fixed top-6 right-6 bg-green-600 text-white px-4 py-2 rounded shadow-lg z-50 hidden"></div>
@endsection
