...
<div class="min-h-screen bg-gray-50">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {{-- Nút Quay Lại --}}
        <div class="mb-6">
            <a href="{{ route('customer.orders.index') }}" class="inline-flex items-center text-orange-600 hover:text-orange-700 font-medium p-2 -ml-2">
            ...
            <div class="p-6 border-b">
                <div class="flex flex-col sm:flex-row justify-between sm:items-start">
                    ...
                    <div id="order-status-badge" class="mt-4 sm:mt-0">
                        <span class="text-sm font-semibold px-4 py-2 rounded-full capitalize" style="background-color: {{ $order->status_color['bg'] }}; color: {{ $order->status_color['text'] }};">
                            {{ $order->status_text }}
                        </span>
                    </div>
                </div>
                                ...
                                <div class="flex items-start space-x-4 p-4 border rounded-lg bg-white hover:shadow-sm">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="flex justify-end mt-3">
                                            <button type="button" class="view-product-details-btn inline-flex items-center justify-center rounded-md text-sm font-medium h-8 px-3 bg-gray-100 text-gray-800 hover:bg-gray-200" data-details='@json($modalData)'>
                                                Xem chi tiết
                                            </button>
                                        </div>
                                    </div>
                    ...
                    <div>
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                <div><span class="text-gray-500">Tên người nhận:</span><p class="font-semibold">{{ $order->customerName }}</p></div>
                                <div><span class="text-gray-500">Số điện thoại:</span><p class="font-semibold">{{ $order->customerPhone }}</p></div>
                                <div class="md:col-span-2"><span class="text-gray-500">Email:</span><p class="font-semibold">{{ $order->customer->email ?? 'Không có' }}</p></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid md:grid-cols-2 gap-x-8 gap-y-6">
                        <div>
                             ...
                             <div class="bg-gray-50 p-4 rounded-lg space-y-3 text-sm">
                                ...
                                @if($order->discount_amount > 0)<div class="flex justify-between text-green-600"><span>Giảm giá ({{ optional($order->discountCode)->code }}):</span><span>-{{ number_format($order->discount_amount, 0, ',', '.') }}đ</span></div>@endif
    ...
    <div id="product-details-modal" class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 transition-opacity duration-300 opacity-0 pointer-events-none">
        <div id="modal-content" class="bg-white rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto transform scale-95 transition-transform duration-300">
